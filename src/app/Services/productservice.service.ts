  ...
  cartItems:any[]=[]
    constructor(private toastr: ToastrService) {
      this.productList = this.productList.map(item => {
        ...
        // Add 'quantity' property with default value  1
        // add 'incart' flag with default value  false
      });
    ...
    {
      const details= this.productList.find((element)=>
        element.id ==id
      )
      console.log(details)
      return details
    ...
    getCartItems(){
      ...
      this.toastr.success("Cart items uploaded successfully",'success')
    ...
    addToCart(product: any ) {
      ...
      if (index === -1) {
        this.cartItems.push({ ...product, incart: true });
        this.toastr.success("added successfully1",'success')
        // Add the product to cart
        product.incart = true;

       // Set the product's inCart property to true
      } else {
        this.removeFromCart(product.id);
        // Call remove if the product is already in the cart
      }
    ...
    removeFromCart(productId: number ) {
      ...
      if (itemIndex > -1) {
        ...
        // Set the incart flag to false for the product

        this.cartItems.splice(itemIndex, 1);
        this.toastr.success("Removed successfully!",'success')

         // Remove the product from cart
         const productIndex = this.productList.findIndex(item => item.id === productId);
         if (productIndex !== -1) {
           // Use Object.assign() to trigger Angular's change detection
           this.productList[productIndex] = Object.assign({}, this.productList[productIndex], { incart: false });
